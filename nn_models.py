...
class GNNLayer(nn.Module):
    def __init__(self, nIn, nOut):
        super().__init__()
        self.projection = nn.Linear(nIn, nOut)
...
class BBTT_GNN(nn.Module):
    def __init__(self, nIn=4, nHidden=12):
        super().__init__()
        ...
        self.fc_0 = nn.Linear(nHidden*self.adj.size()[0], nHidden*2)
        self.fc_1 = nn.Linear(nHidden*2, 2)

    def forward(self, x):
        x = F.leaky_relu(self.gnn_0(x, self.adj), 0.1)
        x = F.leaky_relu(self.gnn_1(x, self.adj), 0.1)
        x = F.leaky_relu(self.gnn_2(x, self.adj), 0.1)
        x = F.leaky_relu(self.gnn_3(x, self.adj), 0.1)
        x = torch.flatten(x, start_dim=1)
        # x = torch.mean(x, dim=1, keepdim=True)
        x = F.leaky_relu(self.fc_0(x), 0.1)
...
class BBTT_GATNN(nn.Module):
    def __init__(self, nIn=4, nHidden=8):
        ...
        self.fc_0 = nn.Linear(nHidden*6, nHidden*2)
        self.fc_1 = nn.Linear(nHidden*2, 2)
